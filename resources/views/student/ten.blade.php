  ...
  <table id="datatb" class="table table-striped table-hover table-responsive">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$user->city}}</td>
            <td>{{$user->country}}</td>
            <td><a href="{{ route('sms.show', $user->id)}}" class="btn btn-success">Show</a></td>
             <td><a href="{{ route('sms.edit', $user->id)}}" class="btn btn-primary">Edit</a></td>
             <td>
                <form action="{{ route('sms.destroy', $user->id)}}" method="post">
                  @csrf
                  @method('DELETE')
